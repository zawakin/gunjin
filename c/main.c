...
void initialdata(void) ;  /* �����ݒ��f�[�^�̍쐬 */
int loadoption(int *classnum, int *height, int *width, opt option[], char datname[]) ;  /* �ݒ��̓ǂݍ��� */ /* return = -1: �ǂݍ��ݎ��s */
void saveoption(int classnum, int height, int width, opt option[], char datname[]) ;  /* �ݒ��̕ۑ� */
void view(int classnum, int board[][16], int turn, int height, int width) ;  /* �Ֆʂ̕\�� */
void setting(int classnum, int board[][16], int turn, int height, int width, opt option[]) ;  /* �z�u */
int movabledomain(int classnum, int pro[], int board[][16], int domain[][16], int turn, int height, int width, opt option[]) ;  /* �ړ��\�̈� */ /* return = -1: �����ȑI�� */
void emphasizedview(int classnum, int board[][16], int domain[][16], int turn, int height, int width) ;  /* �ړ��\�̈��̕\�� */
int movement(int classnum, int pro[], int post[], int board[][16], int domain[][16], int height, int width, opt option[]) ;  /* �ړ� */ /* return = -1: �����Ȉړ� */
void fight(int classnum, int pro[], int post[], int board[][16], int height, int width, opt option[]) ;  /* �퓬 */
int victory(int classnum, int board[][16], int height, int width) ;  /* return = 1: ���U�̏���, 2: ���U�̏���, 3: �������� */
void allview(int classnum, int board[][16], int turn, int height, int width) ;  /* �S���̕\�� */
	...
	if (turn % 2 == 1){
		...
		do {
			...
			if (((pro >= 0 && pro <= classnum) && ((post[0] >= height/2 && post[0] < height) && (post[1] >= 0 && post[1] < width))) && !((pro == 15 || pro == 16) && (post[0] == height/2 && (post[1] == 1 || post[1] == width-2)))){
				...
				if (num == 0){
					printf("���̔z�u�ł��낵���ł����H  yes: 1, no: 0\n\n") ;
					scanf("%d", &yes) ;
				}
			}
		} while (yes == 0) ;
